...
@Component({
  selector: 'app-edit-estimates',
  templateUrl: './edit-estimates.component.html',
  styleUrls: ['./edit-estimates.component.css']
})
export class EditEstimatesComponent implements OnInit {
  tots:any;
...
cals(ServiceData){
  ...
  ServiceData.total =parseInt(( ServiceData.vatper)) +parseInt( (ServiceData.subtotal));
} 
getDataIndividual(){

  var TicketId= localStorage.getItem("Estimate_ID")
  let data={
    "Estimate_ID":TicketId
  }
  this.web3.getEstimartesID(data).subscribe((res)=>{
    console.log(res);
...
this.web3.updateEstimartes(formData).subscribe(res=>{
  console.log("res",res);
